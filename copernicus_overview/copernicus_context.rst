...
-  CEMS Mapping including Rapid Mapping; :doc:`Copernicus Risk and Recovery Mapping<risk_recovery_mapping>` (CEMS RRM); 
   ...
   service and the Risk and Recovery Mapping are both EO based services.

-  :doc:`Early Warning and Monitoring<early_warning_monitoring>` with both EU wide and global services
   with monitoring and forecasting components. For instance, the
   European Drought Observatory (EDO) is continuously extended to the
   global scale (GDO for droughts); the same applies concerning Flooding
   and Forest Fires (GloFAS for floods, GWIS for forest fires).

-  :doc:`Exposure Mapping service<exposure_mapping>`; the goal is the periodic production of
